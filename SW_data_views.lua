...
local function sortByNumber(a, b)
    local numStartA, numEndA = string.find(a, "%d+")
    local numStartB, numEndB = string.find(b, "%d+")
    local numA = tonumber(string.sub(a, numStartA, numEndA))
    local numB = tonumber(string.sub(b, numStartB, numEndB))
    return numA > numB -- 注意此处返回值为大于号
end
...
function SW_DoUnitMouseOver(b)
	...
	-- 获取当前监视框下拉菜单的数值
	local InfoTypesNum = SW_GetBarSettings("SW_BarFrame1").IN

	-- 总伤害
	local totalnum
	local damage = unitData:getDmgDone()
	-- 有效治疗
	local EffectiveHeal = unitData:getEffectiveHealDone()

	if InfoTypesNum == 1 or InfoTypesNum == 2 then
		totalnum = damage
	elseif InfoTypesNum == 3 or InfoTypesNum == 4 then
		totalnum = EffectiveHeal
	end
	...
	-- 鼠标提示
	-- DPS unitData:getDPS()
	-- 有效治疗量 unitData:getEffectiveHealDone()
	-- 实际的治疗量 unitData:getRawHealDone()
	-- 过量治疗百分比  unitData:getOHInFPercentDone()
	-- 暴击 unitData:getDmgCrit()
	-- 总伤害 unitData:getDmgDone()
	
	GameTooltip:SetOwner(b, "ANCHOR_LEFT");
	GameTooltip:AddLine(unitName);
	if InfoTypesNum == 1 or InfoTypesNum == 2 then
		GameTooltip:AddDoubleLine("总伤害", unitData:getDmgDone(), 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("伤害暴击", unitData:getDmgCrit().."%", 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("DPS", unitData:getDPS(), 1, 1, 1, 1, 1, 1);	
	elseif InfoTypesNum == 3 or InfoTypesNum == 4 then
		GameTooltip:AddDoubleLine("有效治疗", unitData:getEffectiveHealDone().." ("..Round(unitData:getEffectiveHealDone() / unitData:getRawHealDone() * 100,1).."%)" or 0 , 0, 1, 0, 1, 1, 1);
		GameTooltip:AddDoubleLine("过量治疗", tonumber(unitData:getRawHealDone() - unitData:getEffectiveHealDone()).." ("..unitData:getOHInFPercentDone().."%)", 0.8, 0.53, 0.53, 1, 1, 1);
		GameTooltip:AddDoubleLine("实际治疗", unitData:getRawHealDone(), 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("治疗暴击", unitData:getHealCrit().."%", 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("HPS", unitData:getHPS(), 1, 1, 1, 1, 1, 1);
	end

	--排序
	for k,v in pairs(skill) do
		local tmpVal
		local spell = SW_StrTable:getStr(k)
		...
		if tmpVal and tmpVal > 0 then
			local value = tmpVal.."|"..spell
			table.insert(SW_GameTooltipSortkeys, value)
			table.sort(SW_GameTooltipSortkeys, sortByNumber)
		end
	end

	if table.getn(SW_GameTooltipSortkeys) > 0 then
		GameTooltip:AddLine(" ")
		GameTooltip:AddLine("详情")
	end
	
	--根据伤害值从大到小显示详情
	for i,v in ipairs(SW_GameTooltipSortkeys) do
		local num, name = string.match(v, "(%d+)|(.+)")
			num = tonumber(num)
			local percent = Round(num / totalnum * 100,1) or 0
			local str = string.format("%s (%.1f%%)", num, percent)
		
		
		if name and num > 0 then
			GameTooltip:AddDoubleLine(name, str, 1, 1, 1, 1, 1, 1)
		end
	end

	if InfoTypesNum == 5 or InfoTypesNum == 8 then
		GameTooltip:AddLine(" ");
		GameTooltip:AddLine(SW_TT_UNIT_IN);
		GameTooltip:AddDoubleLine("伤害", unitData:getDmgRecieved(), 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("有效治疗", unitData:getEffectiveHealRecieved(), 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("实际治疗", unitData:getRawHealRecieved(), 1, 1, 1, 1, 1, 1);
		GameTooltip:AddDoubleLine("过量治疗(战斗中)", unitData:getOHInFPercentRecieved().."%", 1, 1, 1, 1, 1, 1);
	end
